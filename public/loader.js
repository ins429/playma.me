...
;(function(Promise) {
  ...
  function IALoader(canvas, game, callbacks, scale) {
        ...
        loading
          ...
          .then(
            function(data) {
              ...
              module = metadata.getElementsByTagName('emulator').item(0)
                .textContent
              return fetch_file(
                'Emulator Metadata',
          ...
          .then(
            function(data) {
              ...
              var emulator_start_item = metadata
              ...
              if (module && module.indexOf('dosbox') === 0) {
                ...
                module &&
                (module.indexOf('np2-') == 0 ||
                  module.indexOf('xmil-') == 0 ||
                  module.indexOf('vmac-') == 0)
              ) {
                if (!emulator_start_item) {
                  throw new Exception(
                    "Error: this item does not have an 'emulator_start' metadata value; I don't know what to run."
                  )
                }
                config_args.push(
                  cfgr.autoLoad('/emulator/' + emulator_start_item.textContent),
    ...
    function get_dosbox_files(cfgr, metadata, modulecfg, filelist) {
      ...
      meta_props_matching(meta, /^dosbox_drive_([a-zA-Z])$/).forEach(function(
        result
      ) {
        var key = result[0],
          match = result[1]
    ...
    function get_vice_files(cfgr, metadata, modulecfg, filelist) {
      ...
      meta_props_matching(meta, /^vice_drive_([89])$/).forEach(function(
        result
      ) {
        var key = result[0],
          match = result[1]
    ...
    function get_pce_files(cfgr, metadata, modulecfg, filelist) {
      ...
      meta_props_matching(meta, /^pce_drive_([a-zA-Z0-9]+)$/).forEach(function(
        result
      ) {
        var key = result[0],
          periph = result[1][1]
    ...
    function get_np2_files(cfgr, metadata, modulecfg, filelist) {
      ...
      bios_files.forEach(function(fname, i) {
        if (fname) {
          var title = 'ROM File (' + (i + 1) + ' of ' + bios_files.length + ')',
            mounter = fname.endsWith('.zip') ? cfgr.mountZip : cfgr.mountFile
      ...
      bios_files.forEach(function(fname, i) {
        if (fname) {
          var title = 'ROM File (' + (i + 1) + ' of ' + bios_files.length + ')',
            mounter = fname.endsWith('.zip') ? cfgr.mountZip : cfgr.mountFile
          files.push(
            mounter('xmil', cfgr.fetchFile(title, get_bios_url(fname)))
          )
        }
      })
      var meta = dict_from_xml(metadata),
      ...
      bios_files.forEach(function(fname, i) {
        if (fname) {
          var title = 'ROM File (' + (i + 1) + ' of ' + bios_files.length + ')',
            mounter = fname.endsWith('.zip') ? cfgr.mountZip : cfgr.mountFile
          files.push(
            mounter('minivmac', cfgr.fetchFile(title, get_bios_url(fname)))
          )
        }
      })
      var meta = dict_from_xml(metadata),
  ...
  BaseLoader.nativeResolution = function(width, height) {
    ...
    return {
      nativeResolution: { width: Math.floor(width), height: Math.floor(height) }
    }
  }
  ...
  function NP2Loader() {
    ...
    if (!config.emulatorStart) {
      throw new Error(
        'You must specify an autoLoad value in order to start this emulator. Try the name of the disk image.'
      )
    }
    config.emulator_arguments = build_np2_arguments(
      config.emulatorStart,
      config.files,
      config.extra_np2_args
    )
  ...
  var build_mame_arguments = function(
    ...
    if (
      ...
      args.push(
        '-resolution',
        [
          native_resolution.width * scale,
          native_resolution.height * scale
        ].join('x')
      )
    }
  ...
  function SAERunner(canvas, game_data) {
    ...
    for (var i = 0; i < Object.keys(game_data.floppy).length; i++) {
      ...
      this._cfg.floppy.drive[i].file.size = this._cfg.floppy.drive[
        i
      ].file.data.length
    }
  }
  ...
  function Emulator(canvas, callbacks, loadFiles) {
    ...
    this.setScale = function(_scale) {
      console.warn(
        'setScale method is deprecated; use the BaseLoader.scale method instead'
      )
      return this
    }
